...
class WorkManagerActivty : ComponentActivity() {
/*
...
//        )
        ...
        val colorFilterWorkerRequest = OneTimeWorkRequestBuilder<ColorFilterWorker>()
            .build()
        ...
        setContent {
            ...
            val filterInfo = remember(key1 = workInfos) {
                workInfos?.find { it.id == colorFilterWorkerRequest.id }
            }


            val imageUri by derivedStateOf {
                val downloadUri = downloadInfo?.outputData?.getString(
                    WorkerKeys.IMAGE_URI
                )?.toUri()
                val filterUri = filterInfo?.outputData?.getString(
                    WorkerKeys.FILTER_URI
                )?.toUri()
